...
public class UpdateVersionController {
    ...
    public void normalCheckUpdateInfo(String content) {
        ...
        checkVersionTask( content);

    }

    public void forceCheckUpdateInfo(String content) {//强制更新一般不用
        versionCode = getVerCode(context);//等于19
        info = new AppUpdateInfo();
        info.setUrl("http://helloyun.cn:8080/apk/aizy.apk");
        info.setVercode(2);//每次更新都靠它
        info.setVername("2.0");//版本名字
        info.setApkname("com.hellotext.1309171635.apk");
        info.setAppname("AI志愿");
        info.setForceUpp("yes");
        info.setUppcontent(content);//更新内容
        showUpdataDialog();
    }
     ...
     * 步骤一：获取版本信息
     */
    private void checkVersionTask(String content) {
        //网络加载获取app新版版本信息
        //这里不做请求直接赋值
        info = new AppUpdateInfo();
        info.setUrl("http://helloyun.cn:8080/apk/aizy.apk");
